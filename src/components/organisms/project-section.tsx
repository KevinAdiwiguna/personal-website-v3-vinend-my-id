			...
			{dataProject && dataProject?.length > 0 && (
				<div className="mt-16 md:mt-20" id="blogs">
					<section className="border-b border-neutral-800 pb-6 border-dashed">
						...
						<div className="basic-description-color mt-2">My project is a place where I share my thoughts, ideas, and experiences. I write</div>
					</section>
									...
									<div className="mb-5 bg-[#1E1E1E] rounded-xl transition-all duration-300 shadow-sm group relative flex flex-col border dark:border-neutral-800 h-[400px] w-full">
										<div className="duration-500 relative rounded-xl" style={{ height: 400, overflow: "hidden" }}>
											<div style={{ height: 400, overflow: "hidden" }}>
												<Image alt={res.title} loading="lazy" width={10000} height={10000} className="duration-700 ease-in-out scale-100 blur-0 grayscale-0 w-full h-full transform transition-transform group-hover:scale-105 group-hover:blur-sm" src={res.thumbnail} />
											</div>
											...
											<div className="flex flex-wrap gap-2">
												{res?.technologies?.length > 0 &&
													res.technologies.map((tech) => {
														return <Image width={25} height={25} alt={tech.technology?.tech || ""} key={tech.technology?.id} className="px-1" src={tech.technology?.images || ""} />;
													})}
											</div>
											<div className="flex flex-col justify-end">
												<div className="flex flex-col space-y-3">
													<h3 className="font-sora text-lg font-medium text-neutral-100 group-hover:underline group-hover:underline-offset-4">{res.title}</h3>
												...
												<div className="flex justify-between gap-4 text-neutral-400 px-0.5">
													<div className="overflow-hidde flex justify-center gap-x-1 sm:gap-x-3">
														<Image alt={res.user.name || res.user.email || ""} loading="lazy" width="25" height="25" decoding="async" className="duration-700 ease-in-out scale-100 blur-0 grayscale-0 rounded-full min-w-8 min-h-8 max-w-8 max-h-8" src={res?.user?.image || "https://cdn.fakercloud.com/avatars"} />
														<p>{res.user.name}</p>
													</div>
					...
					<ActionButton to="/projects" className="font-semibold flex items-center gap-0 px-0 pr-2 justify-center my-0 py-0 hover:underline duration-300" rightIcon={<FaAngleRight />}>
