...
import { useChangeThemes } from "@/hooks/use-change-themes";

export const LeftBar = ({ blogCount, projectCount }: { blogCount: number; projectCount: number }) => {
	const { toggleTheme, isDark } = useChangeThemes();
	return (
		<aside className="sticky top-0 hidden h-screen border-r border-neutral-800 lg:block dark:border-neutral-700">
			<div className="mr-8 flex h-screen flex-col items-end justify-center gap-4">
				...
				{NAVIGATION.map((res, key) => (
					<ActionButton tooltip={res.tooltip} key={key} to={res.url} className="cursor-pointer rounded-md p-3 navigation-button">
						{res.icon}
					</ActionButton>
				))}
				{blogCount > 0 && (
					<ActionButton tooltip={"Blogs"} to="/#blogs" className="cursor-pointer rounded-md p-3 navigation-button">
						<FaBloggerB />
					</ActionButton>
				)}
				{projectCount > 0 && (
					<ActionButton tooltip={"Project"} to="/#project" className="cursor-pointer rounded-md p-3 navigation-button">
						<FaBloggerB />
					</ActionButton>
				)}
			</div>
